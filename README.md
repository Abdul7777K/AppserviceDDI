...
provider "azurerm" {
  ...
  # App Service Plan for SKU_name Possible values include B1, B2, B3, D1, F1, I1, I2, I3, I1v2, I2v2, I3v2, I4v2, I5v2, I6v2, P1v2,     P2v2, P3v2, P0v3, P1v3, P2v3, P3v3, P1mv3, P2mv3, P3mv3, P4mv3, P5mv3, S1, S2, S3, SHARED, EP1, EP2, EP3, WS1, WS2, WS3, and Y1 Tiers `use_32_bit_worker_process` must be set to `true`.
  # os_type for possible values include 'Windows, Linux, WindowsContainer'
  ...
  # By default App Insight and log analytics resource is created by this module. 
  ...
  # Specifies log analytics workspace retention period in days using `retention_in_days`. Default 90.
  ...
  # Tagging
## Mandatory Tags
| Key | Possible Value(s) |
|--|--|
| Environment| dev, qa, demo, prod |
| Business Criticality|  low, medium, high, critical |
| Workload | Identity |
| Operations Team| systems engineering, network engineering, security team, <$devteam>|
| Owner| <$owner>**|


> Example: $devteam: Carbon _(do not add "ProdDev")_
> Example: $owner: john.smith _(must in format firstname.lastname, do not add @ddiworld.com)_

 **Owner **must** be a named individual in the product or platform owner role

## Optional Tags
Additional tags may be added at the preference of the deploying group. For example:
| Key | Possible Value(s) |
|--|--|
| Service*| pinpoint, dns, firewall|
 *Service could contain multiple tags, and must reflect all the services that utilise a given resource for dependency mapping
...
The pricing tier (B1, B2, B3, D1, F1, I1, I2, I3, I1v2, I2v2, I3v2, I4v2, I5v2, I6v2, P1v2, P2v2, P3v2, P0v3, P1v3, P2v3, P3v3, P1mv3, P2mv3, P3mv3, P4mv3, P5mv3, S1, S2, S3, SHARED, EP1, EP2, EP3, WS1, WS2, WS3, and Y1) of an App Service plan determines what App Service features you get and how much you pay for the plan. There are a few categories of pricing tiers:

- **Shared compute**: `F1` and `D1`, the two base tiers, runs an app on the same Azure VM as other App Service apps, including apps of other customers. These tiers allocate CPU quotas to each app that runs on the shared resources, and the resources cannot scale out.
...
| Name | Description
|--|--
os_type for possible values include 'Windows, Linux, WindowsContainer' default is set to "Windows"
SKU_name specifier of the resource SKU. Available SKU are _F1(Free), D1(Shared), B1(Basic Small), B2(Basic Medium), B3(Basic Large), S1(Standard Small), P1V2(Premium V2 Small), P1V3(Premium V3 Small), P2V3(Premium V3 Medium), P3V3(Premium V3 Large), PC2 (Premium Container Small), PC3 (Premium Container Medium), PC4 (Premium Container Large), I1 (Isolated Small), I2 (Isolated Medium), I3 (Isolated Large), I1v2 (Isolated V2 Small), I2v2 (Isolated V2 Medium), I3v2 (Isolated V2 Large). Valid accepted values are: `B1`, `B2`, `B3`, `D1`, `F1`, `FREE`, `I1`, `I1v2`, `I2`, `I2v2`, `I3`, `I3v2`, `P1V2`, `P1V3`, `P2V2`, `P2V3`, `P3V2`, `P3V3`, `PC2`, `PC3`, `PC4`, `S1`, `S2`, `S3`, `SHARED_`
per_site_scaling_enabled | Can Apps assigned to this App Service Plan be scaled independently? If set to false apps assigned to this plan will scale to all instances of the plan
...
log_analytics_workspace_name | The name of the log analytics workspace default is set to empty, if this variable is pass then it will create the new log analystics workspace, we can just pas the existing log analytics workspace id if for the variable "log_analytics_workspace_id" based on the requirement we have to pass any one of them.
...
`create_app_service_plan`| Whether to create app service plan  | string | `true`
...
`sku_name` | Specifies the app service plan SKU | string | `S1`
`os_type` | Specifies the Os_trype of the app service plan | string | `Windows`
...
`log_analytics_workspace_name` | = Specifies the name of the new log analytics workspace | string | `null`
`log_analytics_workspace_id` | = Specifies the id of the existing log analytics workspace | string | `null`
`log_analytics_sku` | Specifies the loga analytics workspace sku | string | `PerGB2018`
