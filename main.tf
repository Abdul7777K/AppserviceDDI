...
# Reading the existing app service plan
...
resource "azurerm_app_service" "main" {
  ...
  resource_group_name     = local.resource_group_name
  location                = local.location
  app_service_plan_id     = var.create_app_service_plan ? azurerm_service_plan.main[0].id : data.azurerm_service_plan.asp[0].id
  ...
  dynamic "site_config" {
    ...
    content {
      ...
      number_of_workers           = var.per_site_scaling_enabled == true ? lookup(site_config.value, "number_of_workers") : null
...
resource "azurerm_log_analytics_workspace" "law" {
  count               = var.application_insights_enabled && var.application_insights_id == null ? 1 : 0
  name                = lower(format("law-%s", var.log_analytics_workspace_name))
  location            = local.location
  resource_group_name = local.resource_group_name
  sku                 = var.log_analytics_sku
  retention_in_days   = var.retention_in_days
  tags                = merge({ "ResourceName" = "${var.log_analytics_workspace_name}" }, var.tags, )
}
...
resource "azurerm_application_insights" "main" {
  ...
  workspace_id        = var.log_analytics_workspace_name == "" ? var.log_analytics_workspace_id : azurerm_log_analytics_workspace.law[0].id
...
resource "azurerm_app_service_virtual_network_swift_connection" "main" {
  ...
  subnet_id      = data.tfe_outputs.vnetddi.Values.subnet_info.name["subnet1"].id
}
...
resource "azurerm_private_endpoint" "my_private_endpoint" {
  ...
  location            = local.location
  resource_group_name = local.resource_group_name
  subnet_id           = data.tfe_outputs.vnetddi.Values.subnet_info.name["subnet1"].id #data.terraform_remote_state.workspace_b.outputs.subnet_id
