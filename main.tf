...
resource "azurerm_app_service_virtual_network_swift_connection" "main" {
  ...
  subnet_id      = data.tfe_outputs.vnetddi.outputs.subnet_info[0].id
}
...
resource "azurerm_private_endpoint" "my_private_endpoint" {
  ...
  location            = local.location
  resource_group_name = local.resource_group_name
  subnet_id           = data.tfe_outputs.vnetddi.outputs.subnet_info[0].id #data.terraform_remote_state.workspace_b.outputs.subnet_id
